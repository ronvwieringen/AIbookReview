/*
  # Initial Database Schema for AIbookReview

  1. New Tables
    - `profiles` - User profiles with role-based access
    - `books` - Book metadata and information
    - `reviews` - AI-generated reviews and analysis
    - `llm_configs` - LLM model configurations for admin
    - `prompts` - AI prompts management
    - `reading_lists` - User saved books
...
CREATE TYPE review_status AS ENUM ('processing', 'completed', 'failed');
CREATE TYPE llm_status AS ENUM ('active', 'standby', 'inactive');
CREATE TYPE llm_type AS ENUM ('primary', 'backup', 'metadata');
CREATE TYPE prompt_type AS ENUM ('metadata', 'fiction', 'nonfiction');
...
CREATE TABLE IF NOT EXISTS reviews (
  ...
  status review_status DEFAULT 'processing',
...
-- LLM Configurations table (admin only)
...
CREATE TABLE IF NOT EXISTS prompts (
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(type, is_active) WHERE is_active = TRUE -- Only one active prompt per type
);
...
CREATE POLICY "Anyone can read reviews for public books"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM books 
      WHERE books.id = reviews.book_id AND books.is_public = TRUE
    )
  );
...
CREATE POLICY "Authors can read reviews for own books"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM books 
      WHERE books.id = reviews.book_id AND books.author_id = auth.uid()
    )
  );
...
INSERT INTO prompts (name, type, content) VALUES
  ('Metadata Extraction', 'metadata', 'You are an expert book metadata extraction system. Your task is to analyze the provided manuscript text and extract comprehensive metadata about the book.
...
Focus on accuracy and provide detailed, specific information where possible. If certain metadata cannot be determined from the provided text, indicate this clearly in your response.'),
  ...
  ('Fiction Review', 'fiction', 'You are a professional literary critic and book reviewer specializing in fiction. Your task is to provide a comprehensive, balanced, and insightful review of the submitted manuscript.
...
Maintain a professional, constructive tone throughout. Be honest but encouraging, focusing on helping the author improve their work while recognizing its strengths.'),
  ...
  ('Non-Fiction Review', 'nonfiction', 'You are an expert non-fiction book reviewer and editor with extensive experience across multiple non-fiction categories. Your task is to provide a comprehensive, professional review of the submitted non-fiction manuscript.

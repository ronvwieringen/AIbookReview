-- Fix enum type conflicts by safely handling existing types
-- This migration handles the case where types already exist

-- Drop existing types if they exist (in reverse dependency order)
DROP TYPE IF EXISTS sso_provider CASCADE;
DROP TYPE IF EXISTS prompt_type CASCADE;
DROP TYPE IF EXISTS llm_type CASCADE;
DROP TYPE IF EXISTS llm_status CASCADE;
DROP TYPE IF EXISTS review_status CASCADE;
DROP TYPE IF EXISTS book_visibility CASCADE;
...
-- Now ensure all tables exist with proper structure
...
-- Enhanced AI reviews with score breakdown
...
-- Function to check if the current user is an admin (recreate to ensure it exists)
...
DROP POLICY IF EXISTS "Anyone can read visible reader reviews for public books" ON reader_reviews;
DROP POLICY IF EXISTS "Users can create reader reviews" ON reader_reviews;
DROP POLICY IF EXISTS "Users can update own reader reviews" ON reader_reviews;
